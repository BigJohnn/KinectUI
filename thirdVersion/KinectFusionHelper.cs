...
namespace thirdVersion
{
    ...
    public static class Helper
    {
        ...
        public static void OpenBinaryStlMesh(MeshGeometry3D mesh, BinaryReader reader, bool flipAxes)
        {

            char[] header = new char[80];
            header = reader.ReadChars(80);
            ...
            //mesh.triangleIndexes = new List<int>();

            for(int i=0;i<triangles;i++)
            {
                var normal = new Vector3D();
                normal.X = reader.ReadSingle();
                normal.Y = flipAxes ? -reader.ReadSingle() : reader.ReadSingle();
                normal.Z = flipAxes ? -reader.ReadSingle() : reader.ReadSingle();
                //normals[i*3]=normal;
                mesh.Normals.Add(normal);
                
                /*需要特别注意的*/
                //mesh.normals.Insert(mesh.normals.Count, normal);

                for(int j=0;j<3;j++)
                {
                    var vertex=new Point3D();
                    vertex.X = reader.ReadSingle();
                    vertex.Y = flipAxes ? -reader.ReadSingle() : reader.ReadSingle();
                    vertex.Z = flipAxes ? -reader.ReadSingle() : reader.ReadSingle();
                    mesh.Positions.Add(vertex);
                    //vertices[(i*3)+j]=vertex;
                    //mesh.vertices.Insert(mesh.vertices.Count , vertex);
                }
            }
            ...
            if(0==mesh.Positions.Count||0!=mesh.Positions.Count%3||mesh.Positions.Count!=3*mesh.Normals.Count)
            {
                throw new ArgumentException(Properties.Resources.InvalidMeshArgument);
            }
            //mesh.vertices = new List<Vector3>(vertices.ToArray());
            //mesh.normals = new List<Vector3>(normals.ToArray());
            //mesh.triangleIndexes = new List<int>(indices.ToArray());
            //mesh.triangleIndexes = indices;
        }
        ...
        public static void OpenAsciiObjMesh(MeshGeometry3D mesh, BinaryReader reader, bool flipAxes)
        {
            int flag = 0;
